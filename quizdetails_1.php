...
if($_GET['id']=='Technical' || $_GET['id']=='Nontechnical')
{
    ...
    ON s.technology_id=t.id WHERE s.quiz_type='$_GET[id]' ORDER BY $sidx $sord LIMIT $start , $limit";
...
ON s.technology_id=t.id  ORDER BY $sidx $sord LIMIT $start , $limit";
...
if($_GET['status']=="1" || $_GET['status']=="0" )
{
    ...
    ON s.technology_id=t.id WHERE s.is_active='$_GET[status]' AND s.quiz_type='$_GET[id]' ORDER BY $sidx $sord LIMIT $start , $limit";
...
if($_GET['status']==2 && $_GET['id']==2 )
{
    
    $SQL = "SELECT s.id,quiz_date,quiz_name,quiz_number,quiz_type,tname,subcategory_name,quiz_topic,trainername FROM quizdetails s JOIN technology t
    
    ON s.technology_id=t.id WHERE s.is_active='$_GET[status]' ORDER BY $sidx $sord LIMIT $start , $limit";
...
ON s.technology_id=t.id  WHERE s.quiz_date LIKE '%$_GET[qsearch]%' OR s.quiz_name LIKE '%$_GET[qsearch]%' 
    
   OR s.quiz_number LIKE '%$_GET[qsearch]%' OR s.quiz_type LIKE '%$_GET[qsearch]%' OR t.tname LIKE '%$_GET[qsearch]' OR s.trainername LIKE '%$_GET[qsearch]%' OR s.quiz_topic LIKE '%$_GET[qsearch]%' OR s.subcategory_name LIKE '%$_GET[qsearch]%' ORDER BY $sidx $sord LIMIT $start , $limit";
